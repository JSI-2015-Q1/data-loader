...
function JSONSource() {}
JSONSource.prototype = Object.create(DataSource.prototype);
JSONSource.prototype.constructor = JSONSource;
/**
 * @returns {Array} a list of attributes to jkjk
 */
JSONSource.prototype.pathToRecords = []
JSONSource.prototype.handleContents = function(contents, cb) {
  var data = JSON.parse(contents);
  if (this.pathToRecords.length !== 0) {
    data = _.reduce(this.pathToRecords, function(result, attr) {
      return result[attr];
    }, data);
  }

  return _.map(data, this.handleRecord, this);
};
module.exports.JSONSource = JSONSource;
